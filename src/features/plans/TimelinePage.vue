...
<template>
  <div class="page-wrap plan-page">
    ...
    <StatStrip :items="[{ value: inventory.latestSnapshot?.effectiveDate || '待录', label: '库存日期', note: inventory.latestSnapshot ? '按实际库存日期计算' : '请先录入统一库存' }, { value: activeCount, label: '推进中的账号', note: `共 ${projections.length} 个账号` }, { value: dedicatedTotal, label: '专用蛋总数', note: '任务优先消耗' }, { value: regularTotal, label: '普通蛋总数', note: '可任务或出售' }]" />
