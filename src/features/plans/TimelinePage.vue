...
<template><div class="page-wrap plan-page"><nav class="subnav"><RouterLink to="/plans/upgrades">升级计划</RouterLink><RouterLink to="/plans/beasts">神兽计划</RouterLink><RouterLink to="/plans/timeline">联合时间轴</RouterLink></nav><section class="page-intro"><div><h2>联合完成时间轴</h2><p>宝石资金决定周数，神兽任务日期和锁片作为独立约束并列核对。</p></div></section><AccountTimeline :plans="plans"/><section class="timeline-ledger"><div class="table-head"><span>账号</span><span>宝石资金</span><span>神兽资金</span><span>锁片</span><span>神兽日期</span><span>总完成</span></div><RouterLink v-for="plan in plans" :key="plan.accountId" :to="`/accounts/${plan.accountId}`"><b>{{plan.accountId}}</b><span>{{formatCurrency(plan.gemRequiredSilver)}}</span><span>{{formatCurrency(plan.beastRequiredSilver)}}</span><span>{{plan.missingShardCount||'-'}}</span><span>{{beast.find(i=>i.accountId===plan.accountId)?.finishDate}}</span><strong>第 {{plan.finishWeek}} 周</strong></RouterLink></section></div></template>
