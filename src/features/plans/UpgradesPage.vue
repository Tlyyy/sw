...
<template>
  <div class="page-wrap plan-page">
    ...
    <StatStrip :items="[{ value: plans[0]?.accountId, label: '瓶颈号', note: `第 ${plans[0]?.finishWeek} 周` }, { value: '30', label: '装备', note: '每号 6 件' }, { value: formatCurrency(plans.reduce((sum, plan) => sum + plan.gemRequiredSilver, 0)), label: '宝石总缺口', note: '目标 13 段' }, { value: `${market.filter((item) => item.edited).length}`, label: '行情覆盖', note: '来自数据中心' }]" />
    <section class="timeline-section"><AccountTimeline :plans="plans" :selected="selected" /></section>
