...
import { buildTaskPlans, type ScheduledTask } from "../../domain/plans";
...
<template>
  <div class="page-wrap account-page">
    <section class="page-intro"><div><h2>{{ accountId }} 单号下钻</h2><p>从五号推进台进入某一个账号，核对当前神兽任务、库存和后续资料；这里不再制造抽象完成日期。</p></div><div class="account-switch"><RouterLink v-for="account in catalog.data.accounts" :key="account.id" :to="`/accounts/${account.id}`" :class="{ active: account.id === accountId }">{{ account.id }}</RouterLink></div></section>
